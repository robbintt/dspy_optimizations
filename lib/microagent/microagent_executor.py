...
class MicroAgentExecutor:
    ...
    def __init__(self, agent: MicroAgent, model: Optional[str] = None):
        """
        ...
        # Let the harness create its own config, which loads from its YAML.
        # We can pass the harness its own config back for dependency injection.
        harness = MDAPHarness()
        if model:
            harness.config.model = model

        # Create the LLM client and inject it into the harness.
        llm_client = LLMClient(
            model=harness.config.model,
            temperature=harness.config.temperature,
            max_tokens=harness.config.max_tokens
        )
        
        self.harness = MDAPHarness(config=harness.config, llm_client=llm_client)

        logger.info(f"Initialized MicroAgentExecutor with agent: {agent.__class__.__name__}")
        logger.info(f"Configuration: model={harness.config.model}, k_margin={harness.config.k_margin}, "
                   f"max_candidates={harness.config.max_candidates}")
    ...
    async def calibrate(self, calibration_states: List[Any]) -> dict:
        ...
        calibration_results = {
            ...
            'current_k_margin': self.harness.config.k_margin
        }

        logger.info(f"Calibration complete: p={p_estimate:.4f}, k_min={k_min}")
        logger.info(f"Current k_margin={self.harness.config.k_margin}, recommended k_min={k_min}")
    ...
    def update_k_margin(self, new_k_margin: int):
        ...
        """
        old_k = self.harness.config.k_margin
        self.harness.config.k_margin = new_k_margin
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            'model': self.harness.config.model,
            'k_margin': self.harness.config.k_margin,
            'max_candidates': self.harness.config.max_candidates,
            'temperature': self.harness.config.temperature
        }
...
async def execute_agent(agent: MicroAgent,
    ...
    Example:
        ...
        from microagent.micro_agent_executor import execute_agent

        agent = HanoiMDAP()
        trace = await execute_agent(agent, num_disks=5)
        print(f"Solved in {len(trace)-1} steps")
    """
    executor = MicroAgentExecutor(agent, model)
